  ...
  return (
    <section id="help" className="py-20 md:py-32 bg-muted">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-card rounded-3xl p-12 shadow-xl">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              <div className="text-5xl font-bold text-foreground mb-2">
                {stats ? (stats.found_pets ?? 0).toLocaleString("ru") : "10,000+"}
              </div>
              <div className="text-base text-muted-foreground">{t.landing.help.statsFound}</div>
            </div>
            <div>
              <div className="text-5xl font-bold text-foreground mb-2">
                {stats && stats.users_count != null ? stats.users_count.toLocaleString("ru") : "50,000+"}
              </div>
              <div className="text-base text-muted-foreground">{t.landing.help.statsUsers}</div>
            </div>
            <div>
              <div className="text-5xl font-bold text-foreground mb-2">
                {stats ? (stats.cities_count ?? 0).toLocaleString("ru") : "200+"}
              </div>
              <div className="text-base text-muted-foreground">{t.landing.help.statsCities}</div>
            </div>
            <div>
              <div className="text-5xl font-bold text-foreground mb-2">
                {stats && stats.success_rate != null ? `${stats.success_rate}%` : "—"}
              </div>
              <div className="text-base text-muted-foreground">{t.landing.help.statsSuccess}</div>
            </div>
          </div>
        </div>
        )}
