"""Split src/shared/i18n/admin-panel.ts into per-section modules under admin-panel/."""
...
SOURCE = ROOT / "src" / "shared" / "i18n" / "admin-panel.ts"
OUT_DIR = ROOT / "src" / "shared" / "i18n" / "admin-panel"

LOCALES = ("ru", "be", "en")

# Top-level keys in declaration order (must match source).
SECTION_KEYS = [
    "sections",
    "tabs",
    "header",
...
SCALAR_KEYS = {"breedUnknown"}


def export_name(key: str) -> str:
    return f"adminPanel{key[:1].upper()}{key[1:]}Locales"
...
def extract_key_block(locale_body: str, key: str) -> str:
    if key in SCALAR_KEYS:
        ...
        if not m:
            raise ValueError(f"Scalar key not found: {key}")
...
def write_section_module(key: str, per_locale: dict[str, str]) -> Path:
    ...
    if key in SCALAR_KEYS:
        parts = [
            f"/** Admin panel — `{key}` (ru / be / en). */",
            f"export const {name} = {{",
        ]
        for locale in LOCALES:
            parts.append(f"  {locale}: {per_locale[locale]},")
...
def write_composer() -> None:
    ...
    for key in SECTION_KEYS:
        name = export_name(key)
        imports.append(f"import {{ {name} }} from './admin-panel/{key}';")
        ru_lines.append(f"    {key}: {name}.ru,")
        be_lines.append(f"    {key}: {name}.ru,".replace(f"{name}.ru", f"{name}.be"))
        en_lines.append(f"    {key}: {name}.ru,".replace(f"{name}.ru", f"{name}.en"))
...
def main() -> None:
    ...
    print(f"composer admin-panel.ts ({composer_lines} lines)")
