"""Split src/shared/i18n/landing.ts into per-section modules under landing/."""
...
SOURCE = ROOT / "src" / "shared" / "i18n" / "landing.ts"
OUT_DIR = ROOT / "src" / "shared" / "i18n" / "landing"
...
def export_name(key: str) -> str:
    return f"landing{key[:1].upper()}{key[1:]}Locales"
...
def write_composer() -> None:
    ...
    for key in SECTION_KEYS:
        name = export_name(key)
        imports.append(f"import {{ {name} }} from './landing/{key}';")
...
def main() -> None:
    ...
    print(f"composer landing.ts ({composer_lines} lines)")
