...
import { toastApiError } from '@/shared/lib/app-toast';
...
export default function MySheltersPage() {
  ...
  }, [ms.loadError, isVolunteer]);

  useEffect(() => {
    applySeo({
      title: `${ms.title} ќ DorogaDomoy.by`,
  ...
  if (!isVolunteer) {
    return (
      <div className="flex min-h-screen flex-col bg-background pb-24 md:pb-8 dark:bg-background">
        <Header showCitySelector />
        <main className="flex-1 py-6 sm:py-10">
          <div className="page-container">
            <EmptyState
              title={ms.volunteerRequiredTitle}
              description={ms.volunteerRequiredDesc}
              hint={ms.volunteerUpgradeHint}
              icon={<Building2 className="size-7 text-muted-foreground" aria-hidden />}
              action={
                <Button type="button" className={appPrimaryCtaClass} asChild>
                  <Link to="/profile">{ms.volunteerUpgradeCta}</Link>
                </Button>
              }
            />
          </div>
                  ...
                  {list.map((row) => (
                    <li key={row.id} className="px-4 py-4">
                      <div className="hidden items-center gap-4 lg:grid lg:grid-cols-[minmax(200px,2fr)_minmax(90px,1fr)_minmax(130px,1fr)_minmax(160px,1fr)_auto_auto]">
                        ...
                        <span className="truncate text-sm text-muted-foreground">{row.city || 'ќ'}</span>
                      ...
                      <div className="space-y-3 lg:hidden">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-muted-foreground">
                            {row.city || 'ќ'} ќ {kindLabel(row.kind)} ќ {focusLabel(row.animal_focus)}
                          </p>
                        </div>
                        <span
                          className={cn(
