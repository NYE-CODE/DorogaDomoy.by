...
export default function SearchPage() {
  ...
  const toggleLayoutMode = useCallback(() => {
    setLayoutMode((prev) => {
      const next: SearchLayoutMode = prev === 'split' ? 'list' : 'split';
      saveSearchLayoutMode(next);
      return next;
    });
  }, []);
  ...
  const renderContent = () => {
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1">
          <button
            ...
            title={
              layoutMode === 'list'
                ? t.app.searchLayout.mapAndList
                : t.app.searchLayout.listOnly
            }
            className={`relative inline-flex shrink-0 items-center gap-2 rounded-xl border border-transparent px-2 py-2 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground ${layoutMode === 'list' ? 'border-border bg-accent text-foreground' : ''}`}
          >
            {layoutMode === 'list' ? (
      ...
      if (layoutMode === 'list') {
        return (
          <div className="flex-1 flex flex-col min-h-0 w-full">
            <div className="flex-1 min-h-0 overflow-y-auto border-t border-border bg-card">
              {listHeaderContent}
              {listBodyContent}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 w-full">
          <div className="flex-1 min-h-0 relative w-full">
            <div className="absolute inset-0">
              <Suspense fallback={
                <div className="h-full w-full bg-muted/50 dark:bg-muted flex items-center justify-center">
    ...
    return (
      <div className="page-container page-stack flex-1 w-full py-6">
        <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-6 min-h-0">
          <div className={`flex flex-col md:h-[700px] ${layoutMode === 'list' ? 'md:col-span-12' : 'md:col-span-5 lg:col-span-4'}`}>
          ...
          <div className="md:col-span-7 lg:col-span-8 h-[500px] md:h-[700px]">
            <Suspense fallback={
              <div className="h-full w-full rounded-lg border border-border bg-muted/50 dark:bg-muted flex items-center justify-center">
                <div className="text-center text-muted-foreground">
                  <div className="w-8 h-8 border-2 border-border border-t-primary rounded-full animate-spin mx-auto mb-2" />
                  <p className="text-sm">{t.app.mapLoading}</p>
                </div>
