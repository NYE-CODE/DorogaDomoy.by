...
import { blogApi, API_BASE, type BlogPostListItem } from '@/shared/api/client';
...
export default function BlogListPage() {
  ...
  useEffect(() => {
    const desc = truncateMetaDescription(
      `${b.pageSubtitle}. DorogaDomoy.by Ч экосистема помощи животным: поиск, приюты и поддержка в Ѕеларуси.`,
    );
    applySeo({
      title: `${b.pageTitle} Ч DorogaDomoy.by`,
      description: desc,
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="page-container-narrow flex-1 w-full py-10">
        <header className="mb-10">
          <h1 className="typo-h1">{b.pageTitle}</h1>
                    ...
                    <Link to={`/blog/${post.slug}`} className="flex flex-col sm:flex-row sm:min-h-[200px] group">
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <h2 className="typo-h3 group-hover:text-primary transition-colors">
                            {post.title}
                          </h2>
                          {post.excerpt ? (
