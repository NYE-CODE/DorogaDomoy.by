...
import { typoH1, typoH3 } from '@/shared/styles/typography-classes';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function MyPetProfilePage() {
  ...
  const colorsLine = (pet.colors ?? []).filter(Boolean).join(', ') || 'Ч';
  const resolvedSpecies = resolveProfilePetSpecies(pet.species, pet.breed);
  const speciesLine = `${speciesPlainLabel(resolvedSpecies, f)}${pet.breed ? ` Ј ${pet.breed}` : ''}`;
        ...
        <div className="page-container">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 lg:gap-8">
            <div className="space-y-6 lg:col-span-2">
              ...
              {/* ќбложка + шапка профил€; миниатюры сразу под фото Ч без скролла к галерее */}
