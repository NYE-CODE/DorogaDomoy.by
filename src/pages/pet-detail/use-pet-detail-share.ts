...
import { buildPetShareBundle, type PetShareBundle, type PetShareDict } from '@/shared/lib/pet-share-text';
...
export type InstagramGuideState = {
  variant: 'post' | 'story';
  openPath: string;
  cardUrl: string | null;
} | null;

export function usePetDetailShare(pet: Pet, locale: Locale, t: PetDetailT) {
  const [copiedKind, setCopiedKind] = useState<null | 'link' | 'full'>(null);
  const [showShareMenu, setShowShareMenu] = useState(false);
  const [instagramGuide, setInstagramGuide] = useState<InstagramGuideState>(null);
  ...
  const shareBundle: PetShareBundle = buildPetShareBundle(
    pet,
